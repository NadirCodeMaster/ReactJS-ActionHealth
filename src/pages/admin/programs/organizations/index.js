...
import { requestProgramOrganizations } from "api/requests";
...
class Organizations extends Component {
  ...
  componentDidMount() {
    const { program, location, qsPrefix } = this.props;
    window.addEventListener("popstate", this.onPopState);

    if (program) {
      generateTitle(`${program.name} Organizations`);
    }
  ...
  getOrgs = () => {
    ...
    if (program && program.id) {
      ...
      requestProgramOrganizations(program.id, {
        page: currentPage,
        per_page: perPage,
  ...
  render() {
    const { organizationTypes, program, programs, perPage } = this.props;
    ...
    return (
      <React.Fragment>
        <Breadcrumbs>
          ...
          <Breadcrumb path={`/app/admin/programs/${program.id}/organizations`}>
            Organizations
          </Breadcrumb>
        </Breadcrumbs>

        <h1>Organizations</h1>
        <p>Organizations that are eligible for {program.name} assessments.</p>
        <TableSearchBar
                ...
                return (
                  <TableRow key={org.id}>
                    <TableCell component="th" scope="row">
                      <Link to={`/app/admin/organizations/${org.id}`}>{org.name}</Link>
                      <div>
                        {org.city}
                        {org.city && org.state_id && <React.Fragment>{", "}</React.Fragment>}
                        {org.state_id && (
                          <React.Fragment>{org.state_id.toUpperCase()}</React.Fragment>
                        )}
                      </div>
                    </TableCell>
                    <Hidden smDown>
                      <TableCell>
                        ...
                        {isNil(org.parent_organization) && <React.Fragment>--</React.Fragment>}
                      </TableCell>
                    </Hidden>
                    <TableCell>{orgTypeName(org, organizationTypes)}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
