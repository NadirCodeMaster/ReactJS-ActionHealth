...
//
// Form display for questions of type text_checkboxes_v1.
// -----------------------------------------------------------------------------
// Typically rendered inside ./Question (which is then rendered
// inside ./Subsection).
//

export default function QtTextCheckboxesV1({
  ...
  const handleOptionChange = (question, previousSelections, newSelection, otherText) => {
    ...
    // If newSelections contains `none`, we unset all others.
              ...
              {question.value.options.map((opt) => (
                <Fragment key={opt.key}>
                  <FormControlLabel
                    ...
                    disabled={checkboxDisabled(opt.key, readOnly, answerSelections)}
              ...
              {otherKey(question) && (
                <React.Fragment>
                  <FormControlLabel
                    ...
                    disabled={checkboxDisabled(otherKey(question), readOnly, answerSelections)}
...
QtTextCheckboxesV1.propTypes = {
  ...
  storedAnswer: PropTypes.shape(docbuilderAnswerByQuestionTypeShapes.text_checkboxes_v1),
