...
//
// Form display for questions of type text_manual_short_v1.
// --------------------------------------------------------------------
// Typically rendered inside ./Question (which is then rendered
// inside ./Subsection).
//

export default function QtTextManualShortV1({
...
const calculateInitialAnswerVal = memoizee((question, storedAnswer, docbuilderVars) => {
  ...
  // Otherwise, try for a "prepopulate" value from the question.
    ...
    let res = get(question, "value.prepopulate", "");
...
QtTextManualShortV1.propTypes = {
  ...
  storedAnswer: PropTypes.shape(docbuilderAnswerByQuestionTypeShapes.text_manual_short_v1),
