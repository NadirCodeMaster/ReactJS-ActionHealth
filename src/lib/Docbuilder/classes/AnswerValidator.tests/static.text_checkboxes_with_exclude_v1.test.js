...
const questionType = Object.freeze({
  machine_name: 'text_checkboxes_with_exclude_v1'
});
...
test('Answers are valid only when response options are present in question', () => {
  ...
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerWithValidOptions);
  expect(res).toBe(true);

  // Optional question, valid answer
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerWithValidOptions);
  expect(res).toBe(true);

  // Required question, invalid answer
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerWithInvalidOptions);
  expect(res).toBe(false);

  // Optional question, invalid answer
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerWithInvalidOptions);
  expect(res).toBe(false);
});
...
test('Empty answers are valid only question is optional', () => {
  ...
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerNull);
  expect(res).toBe(false);

  // Required question, empty string answer.
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerEmptyArray);
  expect(res).toBe(false);

  // Required question, undefined answer.
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerUndefined);
  expect(res).toBe(false);

  // Required question, totally empty answer.
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, null);
  expect(res).toBe(false);

  // --- OPTIONAL QUESTIONS

  // Optional question, null answer.
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerNull);
  expect(res).toBe(true);

  // Optional question, empty string answer.
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerEmptyArray);
  expect(res).toBe(true);

  // Optional question, undefined answer.
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerUndefined);
  expect(res).toBe(true);

  // Optional question, totally empty answer.
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, null);
  expect(res).toBe(true);
});
...
test('The "other" option requires corresponding text in both optional and required questions', () => {
  ...
  // Required question, valid answer
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerOk);
  expect(res).toBe(true);

  // Required question, answer missing "other" selection
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerMissingSelection);
  expect(res).toBe(false);

  // Required question, answer missing "other" text
  res = AV.text_checkboxes_with_exclude_v1(questionRequired, answerMissingText);
  expect(res).toBe(false);

  // --- OPTIONAL QUESTION TESTS

  // Optional question, valid answer
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerOk);
  expect(res).toBe(true);

  // Optional question, answer missing "other" selection (allowed)
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerMissingSelection);
  expect(res).toBe(true);

  // Optional question, answer missing "other" text (_not_ allowed)
  res = AV.text_checkboxes_with_exclude_v1(questionOptional, answerMissingText);
  expect(res).toBe(false);
});
