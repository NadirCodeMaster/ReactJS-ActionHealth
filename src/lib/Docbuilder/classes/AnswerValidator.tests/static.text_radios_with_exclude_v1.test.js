...
const questionType = Object.freeze({
  machine_name: 'text_radios_with_exclude_v1'
});
...
test('Answers are valid only when response is present in question', () => {
  ...
  res = AV.text_radios_with_exclude_v1(questionRequired, answerValid);
  expect(res).toBe(true);

  // Required question, invalid answer.
  res = AV.text_radios_with_exclude_v1(questionRequired, answerInvalid);
  expect(res).toBe(false);

  // Optional question, valid answer.
  res = AV.text_radios_with_exclude_v1(questionOptional, answerValid);
  expect(res).toBe(true);

  // Optional question, invalid answer.
  res = AV.text_radios_with_exclude_v1(questionOptional, answerInvalid);
  expect(res).toBe(false);
});
...
test('Empty answers are valid only when question is optional', () => {
  ...
  res = AV.text_radios_with_exclude_v1(question, answerEmpty);
  expect(res).toBe(false);

  res = AV.text_radios_with_exclude_v1(questionOptional, answerUsingStandardOption);
  expect(res).toBe(true);
});
...
test('The "other" option requires corresponding text in both optional and required questions', () => {
  ...
  // Required question, valid answer
  res = AV.text_radios_with_exclude_v1(questionRequired, answerOk);
  expect(res).toBe(true);

  // Required question, answer missing "other" selection
  res = AV.text_radios_with_exclude_v1(questionRequired, answerMissingSelection);
  expect(res).toBe(false);

  // Required question, answer missing "other" text
  res = AV.text_radios_with_exclude_v1(questionRequired, answerMissingText);
  expect(res).toBe(false);

  // --- OPTIONAL QUESTION TESTS

  // Optional question, valid answer
  res = AV.text_radios_with_exclude_v1(questionOptional, answerOk);
  expect(res).toBe(true);

  // Optional question, answer missing "other" selection (allowed)
  res = AV.text_radios_with_exclude_v1(questionOptional, answerMissingSelection);
  expect(res).toBe(true);

  // Optional question, answer missing "other" text (_not_ allowed)
  res = AV.text_radios_with_exclude_v1(questionOptional, answerMissingText);
  expect(res).toBe(false);
});
