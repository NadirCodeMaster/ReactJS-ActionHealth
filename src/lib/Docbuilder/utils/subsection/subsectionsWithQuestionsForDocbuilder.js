...
/**
 * Get array of the subsections that HAVE questions from a docbuilder object.
 *
 ...
 * @returns {array}
 */
export default function subsectionsWithQuestionsForDocbuilder(docbuilder, format = 'ids') {
  let res = [];
  for (let i = 0; i < docbuilder.docbuilder_sections.length; i++) {
    ...
    for (let i2 = 0; i2 < section.docbuilder_subsections.length; i2++) {
      let qArr = get(section.docbuilder_subsections[i2], 'docbuilder_questions', []);
      if (qArr.length > 0) {
        if ('ids' === format) {
